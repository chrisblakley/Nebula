        ...
        */
        ...
        public function google_plus($counts=0){
            ...
            <div class="nebula-social-button google-plus-plus-one">
                ...
                <?php if ( ! $this->google_plus_widget_loaded ) : ?>
        ...
        public function linkedin_widget_script(){
            if ( ! $this->linkedin_widget_loaded ){
                ?>
        ...
        public function pinterest_pin($counts=0){ //@TODO "Nebula" 0: Bubble counts are not showing up...
            ...
            <div class="nebula-social-button pinterest-pin">
                ...
                <?php if ( ! $this->pinterest_widget_loaded ): ?>
                    <script type="text/javascript" async defer src="//assets.pinterest.com/js/pinit.js"></script>
                    <?php $this->pinterest_widget_loaded = true; ?>
                <?php endif; ?>
            </div>
            <?php
        }
        ...
        public function is_business_open($date=null, $general=false){
            return $this->business_open($date, $general);
        }

        public function is_business_closed($date=null, $general=false){
            return !$this->business_open($date, $general);
        }
        ...
        public function business_open_until( $weekday ){
            if ( is_business_open() ){
        ...
        public function ip_location($data=null, $ip=false){
        ...
        public function vimeo_meta($id, $meta=''){
            return $this->video_meta('vimeo', $id);
        }

        public function youtube_meta($id, $meta=''){
            return $this->video_meta('youtube', $id);
        }
...
function business_open($date=null, $general=false){
    return nebula()->functions->business_open($date, $general);
}

//If the business is open, return the time that the business closes today
function business_open_until(){
    return nebula()->functions->business_open_until();
}


//Get the relative time of day
function nebula_relative_time($format=null){
    return nebula()->functions->relative_time($format);
}

//Detect location from IP address using https://freegeoip.net/
function nebula_ip_location($data=null, $ip=false){
    return nebula()->functions->ip_location($data, $ip);
}

//Detect weather for Zip Code (using Yahoo! Weather)
//https://developer.yahoo.com/weather/
function nebula_weather($zipcode=null, $data=''){
    return nebula()->functions->weather($zipcode, $data);
}
...
function video_meta($provider, $id){
    return nebula()->functions->video_meta($provider, $id);
}

//Footer Widget Counter
function footer_widget_counter(){
    return nebula()->functions->footer_widget_counter();
}
