...
nebula.optionsInit = function(){
	...
	jQuery('.short-help').each(async function(){
		...
		if ( !jQuery(this).parents('.no-help').length ){
			...
			//if ( jQuery(this).parent().find('.more-help').length ){
				jQuery(this).append('<a class="toggle-more-help" href="#" title="Toggle more information" tabindex="-1"><i class="fa-solid fa-question-circle"></i></a>');
			//}
		}
	});
	...
	jQuery('.tab-pane').each(function(){
		if ( jQuery(this).find('.important-empty').length ){
			if ( !jQuery('.nav-link[href$=' + jQuery(this).attr('id') + '] .empty-important-tab-warning').length ){ //If the warning isn't already showing
				jQuery('.nav-link[href$=' + jQuery(this).attr('id') + ']').append('<i class="fa-solid fa-exclamation-triangle empty-important-tab-warning"></i>');
			}
		} else {
...
nebula.assetScan = function(){
	...
	jQuery('.scan-frontend-assets').on('click', function(){ //Note there are two of these sections (one for styles, one for scripts). This will handle both simultaneously.
		...
		if ( $oThis.attr('data-skip-fetch') !== 'true' ){ //Use Fetch unless it fails
			...
			$oThis.html('<i class="fa-solid fa-spin fa-spinner"></i> Scanning Front-End...');
			...
			fetch(nebula.site.home_url + '?nebula-scan', {
				...
				if ( response.ok ){
					...
					jQuery('.asset-scan-status').html('<strong class="nebula-enabled"><i class="fa-solid fa-check"></i> Automatic scan successful.</strong> You may refresh this page when ready to see available assets.');
				}
			}).catch(function(error){
				$oThis.html('Manually Scan Front-End <i class="fa-solid fa-external-link-alt"></i>');
