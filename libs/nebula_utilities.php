	...
	if ( nebula_vdb_is_same_session() ){ //Same Session
		...
		if ( nebula_vdb_is_page_refresh() ){
			...
			$batch_update_data = nebula_vdb_append_visitor_data(array(
				'pages_viewed' => nebula_url_components('all'),
...
function nebula_vdb_is_page_refresh(){
	$pages_viewed = nebula_vdb_get_visitor_datapoint('pages_viewed');
	if ( $pages_viewed == nebula_url_components('all') ){
		return true;
	}
...
/*
	...
	function project_behavior_points($array){
	    $array['pages_viewed'] = array('/sign-up/', 20); //Visited sign-up page
...
function nebula_url_components($segment="all", $url=null){
	...
	switch ($segment){
		...
		case ('subdomain'):
		case ('sub_domain'):
			if ( isset($host[0]) && $host[0] != 'www' && $host[0] != $sld ){
				return $host[0];
			}
			break;
...
function nebula_render_scss($location_name=false, $location_paths=false, $force_all=false){
	...
	if ( nebula_option('scss', 'enabled') && !empty($location_name) && !empty($location_paths) ){
		...
		if ( nebula_option('dev_stylesheets', 'enabled') ){
			nebula_combine_dev_stylesheets($location_paths['directory'] . '/assets', $location_paths['uri'] . '/assets');
		}
		...
		foreach ( glob($location_paths['directory'] . '/assets/scss/*.scss') as $file ){ //@TODO "Nebula" 0: Change to glob_r() but will need to create subdirectories if they don't exist.
			...
			if ( is_file($file) && $file_path_info['extension'] == 'scss' && $file_path_info['filename'][0] != '_' ){
				$css_filepath = ( $file_path_info['filename'] == 'style' )? $location_paths['directory'] . '/style.css': $location_paths['directory'] . '/assets/css/' . $file_path_info['filename'] . '.css'; //style.css to the root directory. All others to the /css directory in the /assets directory.
...
function nebula_sass_color($color='primary', $theme='child'){
	...
	if ( is_child_theme() && $theme == 'child' ){
		$stylesheets_directory = get_stylesheet_directory() . '/assets';
		$transient_name = 'nebula_scss_child_variables';
	} else {
		$stylesheets_directory = get_template_directory() . '/assets';
