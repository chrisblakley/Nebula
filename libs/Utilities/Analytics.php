	...
	trait Analytics {
		...
		public function ga_parse_cookie(){
			...
			if ( isset($_COOKIE['_ga']) ){
				...
				$cid = $contents['cid'];
			} elseif ( isset($_SESSION) && !empty($_SESSION['nebula_cid']) ){ //@todo "nebula" 0: decommission for session
				$cid = $_SESSION['nebula_cid']; //@todo "nebula" 0: decommission for session
			} else {
				$cid = $this->ga_generate_UUID();
				$_SESSION['nebula_cid'] = $cid; //@todo "nebula" 0: decommission for session
			}
