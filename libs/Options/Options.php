		...
		public function bootstrap($file=false){
			if ( !$this->is_admin_page() ){
				if ( $this->get_option('bootstrap_version') === 'bootstrap3' ){
					...
					if ( $file === 'css' ){
						return wp_register_style('nebula-bootstrap', get_template_directory_uri() . '/assets/css/vendor/bootstrap-reboot-grid.css', null, '4.1.3', 'all'); //Served locally to combine multiple resources (Reboot and Grid)
					} elseif ( $file === 'js' ){
						return false;
					} else {
						return 'grid';
			...
			//Latest (IE10+)
			if ( $file === 'css' ){
				return wp_register_style('nebula-bootstrap', 'https://cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/4.1.3/css/bootstrap.min.css', null, '4.1.3', 'all');
			} elseif ( $file === 'js' ){
