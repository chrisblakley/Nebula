	...
	public function video_meta($provider, $id){
	    ...
	    if ( empty($video_json) ){ //No ?debug option here (because multiple calls are made to this function). Clear with a force true when needed.
	        if ( $provider == 'youtube' ){
	            ...
	            if ( !nebula()->is_available('https://www.googleapis.com/youtube/v3/videos?id=' . $id . '&part=snippet,contentDetails,statistics&key=' . nebula()->option('google_server_api_key')) ){
	...
	public function ip_location($data=null, $ip_address=false){
	    if ( nebula()->option('ip_geolocation') ){
	        ...
	        if ( empty($ip_geo_data) ){
	            ...
	            //Get new remote data
	            if ( empty($_SESSION['nebula_ip_geolocation']) && nebula()->is_available('http://freegeoip.net') ){
	                ...
	                if ( is_wp_error($response) || !is_array($response) || strpos($response['body'], 'Rate limit') === 0 ){
	                    set_transient('nebula_site_available_' . str_replace('.', '_', nebula()->url_components('hostname', 'http://freegeoip.net/json/')), 'Unavailable', MINUTE_IN_SECONDS*5);
	                    return false;
	                }
	...
	public function weather($zipcode=null, $data=''){
	    if ( nebula()->option('weather') ){
	        ...
	        if ( empty($weather_json) ){ //No ?debug option here (because multiple calls are made to this function). Clear with a force true when needed.
	            ...
	            if ( !nebula()->is_available('http://query.yahooapis.com/v1/public/yql?q=' . urlencode($yql_query) . '&format=json') ){
	            ...
	            if ( is_wp_error($response) ){
	                set_transient('nebula_site_available_' . str_replace('.', '_', nebula()->url_components('hostname', 'http://query.yahooapis.com/v1/public/yql')), 'Unavailable', MINUTE_IN_SECONDS*5); //5 minute expiration
	                trigger_error('A Yahoo Weather API error occurred. Yahoo may be down, or forecast for ' . $zipcode . ' may not exist.', E_USER_WARNING);
	                return false;
	            }
